...
namespace M16h {
    ...
    public class TinyStateMachine<TState, TTrigger> {
        ...
        public TinyStateMachine<TState, TTrigger> Tr(
            ...
            if (!canConfigure) {
                throw new InvalidOperationException(
                    $"{nameof(Tr)} cannot be called after {nameof(Fire)} or {nameof(State)} are called."
                );
            }
            ...
            if (!transitions.ContainsKey(from)) {
                ...
                string errorMessage = $"A transition is already defined for state {@from} and trigger {trigger}";
                throw new InvalidOperationException(errorMessage);
            }
